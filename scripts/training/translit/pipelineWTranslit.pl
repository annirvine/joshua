...
my (@CORPORA,$TUNE,$TEST,$ALIGNMENT,$SOURCE,$TARGET,$LMFILE,$GRAMMAR_FILE,$GLUE_GRAMMAR_FILE,$TUNE_GRAMMAR_FILE,$TEST_GRAMMAR_FILE,$THRAX_CONF_FILE, $TRANSLIT_DATA);
my $FIRST_STEP = "FIRST";
my $LAST_STEP  = "LAST";
...
# where transliteration model is stored
my $TRANSLIT_DIR = "$RUNDIR/translit";
...
# By default, don't do transliteration
my $DO_TRANSLIT = 0;

# By default, this isn't a transliteration sub-module
my $TRANSLIT_SUBMOD = 0;
...
my $retval = GetOptions(
  ...
  "translit-submod!" => \$TRANSLIT_SUBMOD,
  "translit-data=s" => \$TRANSLIT_DATA ,
);
...
# make sure a tuning corpus was provided if we're doing MERT and this isn't a transliteration module
if ((! defined $TUNE and ($STEPS{$FIRST_STEP} <= $STEPS{MERT}
						 and $STEPS{$LAST_STEP} >= $STEPS{MERT})) and $TRANSLIT_SUBMOD==0) { 
  print "* FATAL: need a tuning set (--tune)\n";
  exit 1;
}

# make sure if training transliteration model, pointer to data exists
if ($DO_TRANSLIT and ! defined $TRANSLIT_DATA){
    print "* FATAL: need a transliteration corpus to train transliteration model\n";
    exit 1;
}

# make sure a test corpus was provided if we're decoding a test set and this isn't a transliteration module
if ((! defined $TEST and ($STEPS{$FIRST_STEP} <= $STEPS{TEST}
						 and $STEPS{$LAST_STEP} >= $STEPS{TEST})) and $TRANSLIT_SUBMOD==0) {
...
if ($DO_PREPARE_CORPORA) {
    ...
    if ($TRANSLIT_SUBMOD){
	my $prefix = prep_translit_file(@CORPORA);	
    }

  my $prefix = prepare_data("train",\@CORPORA,$MAXLEN, $TRANSLIT_SUBMOD);
...
if (defined $TUNE and $DO_PREPARE_CORPORA) {
  my $prefix = prepare_data("tune",[$TUNE],-1,$TRANSLIT_SUBMOD);
...
if (defined $TEST and $DO_PREPARE_CORPORA) {
  my $prefix = prepare_data("test",[$TEST],-1,$TRANSLIT_SUBMOD);
...
for my $run (1..$OPTIMIZER_RUNS) {
  ...
  #Only do scoring if this isn't a transliteration module
  if($TRANSLIT_SUBMOD==0){
...
## Now, finally, train transliteration model and decode list of OOVs from 1best output above #############################
TRAIN_TRANSLITERATION:
   
if ($DO_TRANSLIT){

for my $run (1..$OPTIMIZER_RUNS) {
  my $testrun = (defined $NAME) ? "$NAME/$run" : "$run";

  system("mkdir -p $TRANSLIT_DIR") unless -d $TRANSLIT_DIR;
  system("mkdir -p $TRANSLIT_DIR/$testrun") unless -d "$TRANSLIT_DIR/$testrun";
  system("mkdir -p $TRANSLIT_DIR/$testrun/data/test") unless -d "$TRANSLIT_DIR/$testrun/data/test";

  #Get a list of OOV words from the 1best output above, want to decode those
  $cachepipe->cmd("get-list-oovs",
      "python $JOSHUA/scripts/training/translit/translitSupport.py 3 test/$testrun/test.output.nbest $TRANSLIT_DIR/$testrun/data/test/test.$SOURCE",
      "$TRANSLIT_DIR/$testrun/data/test/test.$SOURCE.gz");

  $cachepipe->cmd("train-translit-model",
      "perl $JOSHUA/scripts/training/translit/pipelineWTranslit.pl --rundir $TRANSLIT_DIR/$testrun --corpus $TRANSLIT_DATA --source $SOURCE --target $TARGET --maxlen 200 --type hiero --translit-submod --witten-bell --thrax-conf $JOSHUA/scripts/training/translit/thrax-translit.conf",
      "$TRANSLIT_DIR/$testrun/grammar.gz","$TRANSLIT_DIR/$testrun/mert/1/decoder_command");

  #Now extract a 1 best with OOVs tagged from the nbest list, and generate a test.output.1best.translit file with 1best transliterations substituted for tagged OOVs
  $cachepipe->cmd("replace-oovs-newfile",
      "python $JOSHUA/scripts/training/translit/translitSupport.py 4 $TRANSLIT_DIR/$testrun/data/test/test.tok.lc.$SOURCE $TRANSLIT_DIR/$testrun/test/1/test.output.1best test/$testrun/test.output.nbest test/$testrun/test.output.1best.translit",
      "test/$testrun/test.output.1best.translit");  
...
if ($DO_TRANSLIT){
  ...
  $cachepipe->cmd("train-translit-model",
      "perl $JOSHUA/scripts/training/translit/pipelineWTranslit.pl --rundir $TRANSLIT_DIR/$testrun --corpus $TRANSLIT_DATA --source $SOURCE --target $TARGET --maxlen 200 --type hiero --translit-submod --thrax-conf $JOSHUA/scripts/training/translit/thrax-translit.conf",
      "$TRANSLIT_DIR/$testrun/grammar.gz","$TRANSLIT_DIR/$testrun/mert/1/decoder_command");
...
sub prepare_data {
  my ($label,$corpora,$maxlen,$trans) = @_;

  system("mkdir -p $DATA_DIR") unless -d $DATA_DIR;
  system("mkdir -p $DATA_DIRS{$label}") unless -d $DATA_DIRS{$label};

  # copy the data from its original location to our location if not transliteration data
  if ($trans==0){
  ...
  foreach my $lang ($TARGET,$SOURCE,"$TARGET.0","$TARGET.1","$TARGET.2","$TARGET.3") {
	if (-e "$DATA_DIRS{$label}/$prefix.$lang.gz") {
		...
		$cachepipe->cmd("translit-tokenize-$lang-$label",
                "$SCRIPTDIR/training/translit/translitTok.sh $DATA_DIRS{$label}/$prefix.$lang.gz $DATA_DIRS{$label}/$prefix.tok.$lang",
                "$DATA_DIRS{$label}/$prefix.tok.$lang.gz");
	    }
	}
  }
...
# Copy transliteration data from single corpus (list of pairs of transliterated words), make filenames consistent and run 
# python module that splits into train/tune and readies for transliteration translation module (spaces between characters, etc.)
sub prep_translit_file{
    my ($corpora) = @_;
    my $prefix = "corpus.";
  ...
  # copy the data from its original location to our location                                                                                                                                 
    foreach my $ext ($TARGET,$SOURCE) {
        ...
        #my $files = join(" ",@files);                                                                                                                                                       
        if (-e $file) {
          $cachepipe->cmd("translit-copy-$ext",
             "cp $file $DATA_DIRS{train}/$prefix$ext",
			  "$DATA_DIRS{train}/$prefix$ext");
        }

        # append each extension to the corpora prefixes                                                                                                                                      
        if (-e "$DATA_DIRS{train}/$prefix$ext") {
          $cachepipe->cmd("split-translit-data-$ext",
              "python $SCRIPTDIR/training/translit/translitSupport.py 2 $DATA_DIR $ext 15 0",
			  "$DATA_DIRS{train}/$prefix.dev.$ext.", "$DATA_DIRS{train}/$prefix.train.$ext", "$DATA_DIRS{train}/$prefix.test.$ext");
        }
    }
    return $prefix;
